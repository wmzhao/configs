...
get_short_path() {
  ...
  if [[ $LONG_PATH =~ ^$MP ]]; then
    LONG_PATH=${LONG_PATH/$MP/PRJ}
	fi
  if [[ $LONG_PATH =~ ^$MS ]]; then
    LONG_PATH=${LONG_PATH/$MS/SCRATCH}
	fi
  MS1="/prj/llvm-arm/scratch1/weimingz"
  if [[ $LONG_PATH =~ ^$MS1 ]]; then
    LONG_PATH=${LONG_PATH/$MS1/SCRATCH1}
	fi
